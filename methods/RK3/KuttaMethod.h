...
namespace solvers::KuttaMethod
{

template<
    typename BaseSolver
>
class KuttaMethod : public BaseSolver
{
private:
...
public:
    explicit KuttaMethod(equation_type eq) 
    : BaseSolver(eq),
      m_equation{eq},
      m_step{eq.get_step()}
    {}
...
public:
    ...
    )
    {
        ...
        auto k3 = m_equation.f(old_point + m_step, old_solution - m_step * k1 + 2 * m_step * k2);
        
        return old_solution + m_step * (1./6. * k1 + 2./3. * k2 + 1./6. * k3); 
    }
};
